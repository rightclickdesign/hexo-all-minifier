...
const packageVersion = proces.env.npm_package_version;
...
function concatJS(data) {
  ...
  const htmls = {};
  const useVersionedBundle = options.use_versioned_bundle;
  let bundlePath = options.bundle_path;
  if (useVersionedBundle && packageVersion) {
    if (bundlePath.includes('.min.js')) {
      bundlePath = bundlePath.replace(/min.js$/, `${packageVersion}.min.js`);
      log.log('Update Min Bundled Path to: %s', bundlePath);
    } else {
      bundlePath = bundlePath.replace(/.js$/, `${packageVersion}.js`);
      log.log('Update Bundled Path to: %s', bundlePath);
    }
  }
